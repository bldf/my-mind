...
# my-qa-engineer — QA 工程师

在开发任务完成后执行整体质量验证。自动识别项目测试框架。

## 触发条件

由 `/my:ai` 自动调用，当 task 涉及测试或全部开发完成后触发。
...
1. 启动开发服务器
2. 选择浏览器驱动（按优先级）：
   - **检查项目配置**：如 `.codex/rules/testing.md` 中指定了 `browser_driver`，使用用户指定的方式
...
> **用户覆盖**：在 `.codex/rules/testing.md` 中添加 `browser_driver: playwright | chrome-mcp | ask` 可固定选择或设为每次询问。
