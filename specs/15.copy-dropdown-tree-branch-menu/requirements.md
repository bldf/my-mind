...
在编辑器顶部工具栏增加复制数据下拉按钮，支持复制当前导图的 JSON、Markdown、Mermaid 文本；同时将现有左侧一级分支菜单升级为紧凑默认展开的三级 Tree 菜单，让用户可以从原始根节点下的一级、二级和三级导航项快速聚焦对应子树，并可通过单一入口在默认、全部展开、紧凑和全部收起状态间切换。
...
| 2026-06-27 | v2 | 明确 Tree 默认紧凑展开和单按钮展开循环 |
...
- Tree 默认紧凑展开：一级节点始终展示，仍有下钻信息的父级路径默认展开，仅包含单节点终点的父项默认折叠，避免侧栏被单叶子链路拉长。
- Tree 顶部提供一个展开状态循环按钮：默认紧凑态显示 `Expand all branches`，全部展开后显示 `Collapse single-count branches`，紧凑态显示 `Collapse all branches`，全部收起后显示 `Restore default branch expansion`。
- 本期不要求持久化折叠状态；后续如需跨会话记忆展开状态可作为独立增强。

## 用户故事

- 作为导图编辑用户，我希望在顶部工具栏直接复制当前导图数据，以便快速粘贴到文档、聊天窗口或 Mermaid 渲染工具中。
- 作为导图编辑用户，我希望复制按钮悬停后出现格式列表，以便不离开画布就能选择 JSON、Markdown 或 Mermaid。
- 作为导图阅读用户，我希望左侧分支菜单不只展示一级分支，而是以 Tree 方式展示到三级，并能按需展开所有可见层级，以便快速定位较深的主题分组。
- 作为大纲整理用户，我希望 Tree 菜单默认采用紧凑展开并只保留有下级内容的父节点，以便菜单保持紧凑，不被大量叶子节点淹没。
...
8. [F-008] Tree 菜单必须默认紧凑展开，最多展示一级、二级和三级；三级以下节点不展示。
...
14. [F-014] Tree 菜单顶部必须提供单按钮展开循环，在默认紧凑、全部展开、紧凑和全部收起状态之间切换；文档结构变化时当前循环状态必须与新的 Tree 结构同步。
...
- [ ] [AC-006] 深层导图进入分支列表模式后，左侧菜单以 Tree 方式默认紧凑展开，展示原始根节点下一级及符合规则的二级、三级项。
...
- [ ] [AC-011] 展开循环按钮可从默认紧凑态切到全部展开、紧凑、全部收起并恢复默认；当文档新增可展开分支时，当前循环状态会同步到新分支。
- [ ] [AC-012] 相关 React 单元测试、Playwright E2E、`pnpm typecheck`、`pnpm test`、`pnpm e2e`、`git diff --check` 通过。
...
- 暂无。Tree 展示规则按“实现约定”落地；如果后续希望叶子节点全部参与导航或持久化展开状态，可另开变更规格。
