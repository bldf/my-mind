...
- 涉及的技术层
- 总任务数和预估总时间
- 提示用户：**请审查 specs 后，运行 `/my:ai` 或 `/my:autoai` 开始开发**

---

## 变更模式

当输入 `/my:prd --change {N}.{feature-name} 变更内容` 时执行。
...
请产品审查变更后，运行 /my:autoai 继续开发（会跳过已完成任务）
```
...
/my:prd ~/projects/my-app-specs
/my:prd /path/to/project-specs

# 需求变更 — 同样基于项目文件夹，指定要变更的 feature
/my:prd --change 1.user-auth 新增微信扫码登录方式
/my:prd --change 2 ~/projects/my-app-specs
```
